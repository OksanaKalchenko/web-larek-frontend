...
Реализует отображение модального окна, наследует класс Component.Так же предоставляет методы `open` и `close` для управления отображением модального окна. Метод `set content` задает содержимое модального окна, а метод `render` отрисовывает данные и выводит их в модальном окне.
